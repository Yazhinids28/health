...
const Homeopathy = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/predict-result", formData);
      navigate("/result-hom", { state: { result: res.data } });
    } catch (err) {
  ...
  return (
    <div className={`homeopathy-container ${darkMode ? "dark" : ""}`}>
      <div className="form-card">
        <h2 className="form-title">
          <span>💊</span> Homeopathy Health Predictor
        </h2>
