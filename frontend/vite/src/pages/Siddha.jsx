...
import "../styles/siddec.css"; // Use your Siddha-specific CSS
...
const Siddha = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5002/predict-siddha", formData); // Use Siddha endpoint
      navigate("/result-sid", { state: { result: res.data } });
    } catch (err) {
    ...
    <div className={`ayurveda-container ${darkMode ? "dark" : ""}`}>
      <div className="form-card">
        <h2 className="form-title">
          <span>🌿</span> Siddha Health Predictor
        </h2>
